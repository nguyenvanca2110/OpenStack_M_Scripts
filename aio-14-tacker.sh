...
2. create systemclt service script
cp /etc/init.d/neutron-server /etc/init.d/tacker-server
cp /lib/systemd/system/neutron-server.service /lib/systemd/system/tacker-server.service

vi /etc/init.d/tacker-server
#! /bin/sh
### BEGIN INIT INFO
...
SYSTEM_USER=root
SYSTEM_GROUP=root
CONFIG_FILE=/usr/local/etc/${PROJECT_NAME}/${PROJECT_NAME}.conf
LOGFILE=/var/log/${PROJECT_NAME}/${NAME}.log

...... æ∆∑°∫Œ∫–∫Œ≈Õ ∞∞¿∏¥œ ¿≠ ∫Œ∫ŒπÆ ¿≠ ∫Œ∫–∏∏ ∫Ø∞Ê «œµµ∑œ
if [ -z "${NO_OPENSTACK_CONFIG_FILE_DAEMON_ARG}" ] ; then
        DAEMON_ARGS="--config-file=${CONFIG_FILE} ${DAEMON_ARGS}"
fi


vi /lib/systemd/system/tacker-server.service
[Unit]
...
ExecStartPre=/bin/chown root:root /var/lock/tacker /var/log/tacker /var/lib/tacker
ExecStart=/etc/init.d/tacker-server systemd-start
Restart=on-failure
LimitNOFILE=65535
TimeoutStopSec=15
...
## º≠∫ÒΩ∫ µÓ∑œ
systemctl disable tacker-server.service
systemctl enable tacker-server.service

systemctl restart tacker-server.service
systemctl status tacker-server.service

tail -f /var/log/tacker/tacker-server.log


## and db install (don't need)
/usr/local/bin/tacker-db-manage --config-file /usr/local/etc/tacker/tacker.conf upgrade head
